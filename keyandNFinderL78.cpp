...
// ------------------ Estructura para diccionario LZ78 ------------------
struct Entrada {
    int offset;
    int length;
};
...
int main(void) {
    const char* archivoEnc = "Encriptado2.txt";
    const char* archivoPista = "pista2.txt";
    ...
    for (int n = 1; n <= 7 && !encontrado; ++n) {
        for (int K = 0; K <= 255 && !encontrado; ++K) {
            ...
            // buffer temporal para la salida completa
            unsigned char* full_output = (unsigned char*)malloc(1);
            size_t output_size = 0;

            Entrada* dictEntries = (Entrada*)malloc(sizeof(Entrada) * 100000); // diccionario dinámico
            unsigned char* dictBuffer = (unsigned char*)malloc(200000); // buffer de reconstrucción
            int dictCount = 0;
            int dictLen = 0;
            ...
            while (1) {
                ...
                // reconstrucción LZ78
                if (pos > 0) {
                    Entrada prefijo = dictEntries[pos - 1];
                    for (int j = 0; j < prefijo.length; ++j) {
                        unsigned char c = dictBuffer[prefijo.offset + j];
                        full_output = (unsigned char*)realloc(full_output, output_size + 1);
                        full_output[output_size++] = c;
                        ...
                        if (buf_count == pista_len) {
                            ...
                            for (int k = 0; k < pista_len; ++k) {
                                ...
                                if (buffer[idx] != pista[k]) { match = 0; break; }
                            }
                            if (match) { match_this_combo = 1; encontrado = 1; break; }
                        }
                    }
                }

                // añadir símbolo actual
                full_output = (unsigned char*)realloc(full_output, output_size + 1);
                ...
                if (buf_count == pista_len) {
                    ...
                    if (match) { match_this_combo = 1; encontrado = 1; }
                }
                ...
                if (pos > 0) {
                    Entrada prefijo = dictEntries[pos - 1];
                    nueva.offset = prefijo.offset;
                    nueva.length = prefijo.length + 1;
                } else {
                    nueva.offset = dictLen;
                    nueva.length = 1;
                }
                dictEntries[dictCount++] = nueva;
                dictBuffer[dictLen++] = symbol;
